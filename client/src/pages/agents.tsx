...
import { Plus, Trash2, Play, Pause, Bot, Zap, Activity, RefreshCw } from "lucide-react";
...
export default function Agents() {
  ...
  const { agents, projects, addAgent, updateAgent, deleteAgent } = useData();
  ...
  function toggleStatus(agent: any) {
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-3 pt-2">
              <Input placeholder="Agent name" value={name} onChange={e => setName(e.target.value)} data-testid="input-agent-name" />
              <Select value={type} onValueChange={setType}>
                <SelectTrigger data-testid="select-agent-type"><SelectValue placeholder="Agent type" /></SelectTrigger>
                <SelectContent>
                  {agentTypes.map(t => (
              ...
              <Textarea placeholder="What does this agent do?" value={description} onChange={e => setDescription(e.target.value)} className="min-h-[80px]" data-testid="input-agent-description" />
              <Select value={projectId} onValueChange={setProjectId}>
                <SelectTrigger data-testid="select-agent-project"><SelectValue placeholder="Assign to project" /></SelectTrigger>
                <SelectContent>
              ...
              <Button onClick={handleCreate} disabled={!name.trim()} className="w-full" data-testid="button-submit-agent">
                Create Agent
              </Button>
            </div>
          </DialogContent>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            <div className="p-2 rounded-md bg-green-500/10"><Zap className="size-4 text-green-600 dark:text-green-400" /></div>
            <div>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            <div className="p-2 rounded-md bg-blue-500/10"><Activity className="size-4 text-blue-600 dark:text-blue-400" /></div>
            <div>
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            <div className="p-2 rounded-md bg-purple-500/10"><RefreshCw className="size-4 text-purple-600 dark:text-purple-400" /></div>
            <div>
            ...
            <Card key={agent.id} className="group" data-testid={`agent-card-${agent.id}`}>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex items-center gap-1.5 pt-1">
                  <Button size="sm" variant={agent.status === "running" ? "outline" : "default"} className="h-7 text-xs" onClick={() => toggleStatus(agent)} data-testid={`button-toggle-agent-${agent.id}`}>
                    {agent.status === "running" ? <Pause className="size-3 mr-1" /> : <Play className="size-3 mr-1" />}
                    {agent.status === "running" ? "Pause" : "Start"}
                  </Button>
                  <Button size="sm" variant="ghost" className="h-7 text-xs text-muted-foreground hover:text-destructive" onClick={() => { deleteAgent(agent.id); toast({ title: "Agent removed" }); }} data-testid={`button-delete-agent-${agent.id}`}>
